...
class HZmfemDirichlet(PDEBase):
    """
    二维线弹性 (胡张混合元) —— 纯位移边界条件模型
...
class HZmfemConsistentMix(PDEBase):
    """
    二维线弹性 (胡张混合元) —— 混合边界条件 (满足角点相容性条件) 
...
class HZmfemInconsistentMix(PDEBase):
    """
    二维线弹性 (胡张混合元) —— 混合边界条件 (不满足角点相容性条件) 
