...
def run_filter_new_3d_test(config: TestConfig):
    ...
    SF = SensitivityBasicFilter(mesh=mesh, rmin=config.filter_radius, domain=domain)
    H, HS = SF._compute_filter_3d(nx=config.nx, ny=config.ny, nz=config.nz,
                                    hx=config.hx, hy=config.hy, hz=config.hz,
                                    rmin=config.filter_radius)
    H1 = H.toarray()
    ...
    mesh_tet = TetrahedronMesh.from_box(box=[0, 1, 0, 1, 0, 1], nx=10, ny=10, nz=10)
    SF_tet = SensitivityBasicFilter(mesh=mesh_tet, rmin=config.filter_radius, domain=domain)
    H_tet, HS_tet = SF_tet._compute_filter_matrix()
    H_tet_1 = H_tet.toarray()
