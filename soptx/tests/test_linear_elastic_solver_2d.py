...
# K0_top = solvers_top._assemble_global_stiffness_matrix()

# F0_top = solvers_top._assemble_global_force_vector()

# K_top, F_top = solvers_top._apply_boundary_conditions(K0_top, F0_top)

# uh_top = tensor_space_C.function()
# try:
#     uh_top[:] = spsolve(K_top, F_top[:], solver='mumps')
# except Exception as e:
#     raise RuntimeError(f"Direct solver failed: {str(e)}")

# try:
#     uh_top[:], info = cg(K_top, F_top[:], x0=None,
#                     batch_first=True,
...
if __name__ == "__main__":
    ...
    mesh = QuadrangleMesh.from_box(box=[0, domain_x, 0, domain_y], nx=nx, ny=ny)
    test_elastic_with_interpolated_rhs(pde_interpolated_rhs, mesh, p, solver_type='mumps')
