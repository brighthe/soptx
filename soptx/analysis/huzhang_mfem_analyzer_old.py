...
from soptx.functionspace.huzhang_fe_space_old import HuZhangFESpace
...
class HuZhangMFEMAnalyzer(BaseLogged):
    ...
    def assemble_stiff_matrix(self, 
        ...
        if p >= GD + 1:
            ...
            # K_dense = K.toarray().round(4)
            # gdof_sigma = space_sigma.number_of_global_dofs()   
            # J = K[gdof_sigma:, gdof_sigma:]
            # J_dense = J.toarray().round(4)

            # TODO
            J1 = bform3.assembly(format='csr')
            # J1_dense = J1.toarray().round(4)
            # from scipy.sparse import csr_matrix, coo_matrix, bmat, spdiags    
            # K1 = bmat([[M.to_scipy(),   B.to_scipy()],
            #            [B.T.to_scipy(), J1.to_scipy()]], format='csr')
            # K1_dense = K1.toarray().round(4)
            
            from fealpy.sparse.ops import bmat, spdiags
            K     = bmat([[M,   B],
                           [B.T, J1]], format='csr')    
            # K2_dense = K2.toarray().round(4)

            
            # error_k = bm.linalg.norm(K.toarray() - K1.toarray())

            # import numpy as np
            # import os
            # K_np = np.array(K_dense)
            # K1_np = np.array(K1_dense)
            ...
            # # 1. 保存 K_dense (BlockForm 组装结果)
            # np.savetxt(os.path.join(save_dir, "K_BlockForm.csv"), K_np, delimiter=",", fmt="%.4f")
            # print(f"已保存: {os.path.join(save_dir, 'K_BlockForm.csv')}")

            # # 2. 保存 K1_dense (手动组装结果)
            # np.savetxt(os.path.join(save_dir, "K_Manual.csv"), K1_np, delimiter=",", fmt="%.4f")
            # print(f"已保存: {os.path.join(save_dir, 'K_Manual.csv')}")

            # # 3. 保存差异矩阵 (方便快速定位不为0的项)
            # np.savetxt(os.path.join(save_dir, "K_Diff.csv"), Diff_np, delimiter=",", fmt="%.4f")
            # print(f"已保存: {os.path.join(save_dir, 'K_Diff.csv')}")
