...
class DensityTopOptTest(BaseLogged):
    ...
    def run(self, 
        ...
        from soptx.regularization.filter import Filter
        rmin = (0.04 * nx) / (domain_length / nx)
        # density_coords = opt_mesh.interpolation_points(p=1)
        # density_coords = opt_mesh.entity_barycenter('cell')
        
        # from soptx.regularization.matrix_builder import FilterMatrixBuilder
        # fmb = FilterMatrixBuilder(mesh=opt_mesh, rmin=rmin, density_coords=density_coords)
        # H, Hs = fmb._compute_filter_general(
        #                             rmin=rmin,
        #                             domain=opt_mesh.meshdata['domain'],
        #                             density_coords=density_coords)
        # H1, Hs1 = fmb._compute_weigthed_matrix_2d(
        #                             rmin=rmin, 
        #                             nx=nx, ny=ny, 
        #                             hx=domain_length / nx, hy=domain_height / ny,
        #                             density_type='element',
        #                             enable_timing=False
        #                         )
        # H_dense = H.to_dense()
        # H1_dense = H1.to_dense()
        # print(f"error norm: {bm.sum(H_dense - H1_dense)}")
