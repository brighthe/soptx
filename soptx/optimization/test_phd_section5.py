...
class DensityTopOptHuZhangTest(BaseLogged):
    ...
    @run.register('test_subsec5_6_3_lfem_clapmed_beam_2d')
    def run(self) -> Union[TensorLike, OptimizationHistory]:
    ...
    @run.register('test_subsec5_6_3_lfem_bearing_device_2d')
    def run(self) -> Union[TensorLike, OptimizationHistory]:
    ...
    def run(self) -> Union[TensorLike, OptimizationHistory]:
        ...
        self._log_info(f"开始密度拓扑优化, \n"
                ...
                f"平面类型={pde.plane_type}, 外载荷类型={pde.load_type}, 杨氏模量={pde.E}, 泊松比={pde.nu} \n"
                f"网格类型={mesh_type}, 密度类型={density_location}, "
                f"网格尺寸={design_variable_mesh.number_of_cells()}, 密度自由度={rho.shape[0]} \n"
    ...
    @run.register('test_subsec5_6_3_hzmfem_bearing_device_2d')
    def run(self) -> Union[TensorLike, OptimizationHistory]:
        #* 轴承装置结构 bearing_device_2d
        t = -8e-2
        E, nu = 1, 0.5 # 0.3, 0.5
