...
class DensityTopOptHuZhangTest(BaseLogged):
    ...
    def run(self, analysis_method: str = 'huzhangfem') -> Union[TensorLike, OptimizationHistory]:
        ...
        if analysis_method == 'lfem':
            ...
            analyzer = lagrange_fem_analyzer
            analysis_space = analyzer.tensor_space
            TGDOF = analysis_space.number_of_global_dofs()
            self._log_info(f"分析阶段参数, "
                           f"模型名称={pde.__class__.__name__}, 平面类型={pde.plane_type}, "
                           f"外载荷类型={pde.load_type}, 边界类型={pde.boundary_type}, "
                           f"杨氏模量={pde.E}, 泊松比={pde.nu}, "
                           f"离散方法={analysis_method}, 网格类型={mesh_type}, "
                           f"状态变量={state_variable}, "
                           f"分析空间={analysis_space.__class__.__name__}, 次数={analysis_space.p}, 总自由度={TGDOF}")
