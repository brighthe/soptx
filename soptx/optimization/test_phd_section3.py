...
class DensityTopOptTest(BaseLogged):
    ...
    @run.register('test_subsec3_6_4_simply_support_bridge_2d')
    def run(self) -> Union[TensorLike, OptimizationHistory]:
        t = 1.0
        E, nu = 1, 0.3
        domain = [0, 120, 0, 40]
        ...
        from soptx.model.simply_support_bridge_2d_lfem import SimplySupportedBridge2d
        pde = SimplySupportedBridge2d(
                            domain=domain,
                            t=t, E=E, nu=nu, 
                            plane_type=plane_type,
                            enable_logging=False
        ...
        base_dir = current_file.parent.parent / 'vtu'
        base_dir = str(base_dir)
        save_path = Path(f"{base_dir}/subsec3_6_4_simply_support_bridge_2d")
        save_path.mkdir(parents=True, exist_ok=True)    
