...
class DensityTopOptTest(BaseLogged):
    ...
    def run(self, parameter_type: str = 'mbb_2d') -> Union[TensorLike, OptimizationHistory]:
        ...
        if parameter_type == 'mbb_2d':
            ...
            # 单元密度
                # - 四边形网格: 积分精度 q = p+1 就足够
            # 节点密度
                # - 四边形网格: 积分精度 q = p+2 就足够
            space_degree = 1
        ...
        save_path = Path(f"{base_dir}/stop_hu60-10_hd25_p1_r1")
    ...
    @run.register('test_stop_mtop_assembly_2d')
    def run(self):
        """ 测试当单元密度一致时, 单分辨率与多分辨率下的矩阵组装是否一致 """
        domain = [0, 60.0, 0, 10.0]
        T = -1.0
        E, nu = 1.0, 0.3
        ...
        # 单元密度单分辨率情况
        density_location_stop = 'element'
        interpolation_scheme_stop = MaterialInterpolationScheme(
                                        density_location=density_location_stop,
                                        interpolation_method='msimp',
                                        options={
                                            'penalty_factor': penalty_factor,
                                            'void_youngs_modulus': 1e-9,
        ...
        lagrange_fem_analyzer_stop = LagrangeFEMAnalyzer(
                                    ...
                                    space_degree=space_degree,
                                    integration_order=integration_order,
                                    assembly_method=assembly_method_stop,
                                    solve_method='mumps',
                                    topopt_algorithm='density_based',
                                )
        # 单元密度多分辨率情况
        density_location_mtop = 'element_multiresolution'
        interpolation_scheme_mtop = MaterialInterpolationScheme(
                                        density_location=density_location_mtop,
                                        interpolation_method='msimp',
                                        options={
                                            'penalty_factor': penalty_factor,
                                            'void_youngs_modulus': 1e-9,
        ...
        assembly_method_mtop = 'voigt_multiresolution'
        lagrange_fem_analyzer_mtop = LagrangeFEMAnalyzer(
                            ...
                            space_degree=space_degree,
                            integration_order=integration_order,
                            assembly_method=assembly_method_mtop,
                            solve_method='mumps',
                            topopt_algorithm='density_based',
                        )

        self._log_info(f"测试矩阵组装, "
                       f"模型名称={pde.__class__.__name__}, "
                       f"网格类型={mesh_type}, " 
                       f"位移网格尺寸={displacement_mesh.number_of_cells()}, 位移有限元空间阶数={space_degree}, "
                       f"子密度单元数={sub_density_element}, ")
        ...
        error_K = bm.linalg.norm(K_stop.to_dense() - K_mtop.to_dense()) / bm.linalg.norm(K_stop.to_dense())
        print(f"刚度矩阵相对误差: {error_K}")
...
if __name__ == "__main__":
    ...
    # test.run.set('test_stop_mtop_assembly_2d')
