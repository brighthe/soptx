...
from . import contract, layout, schema
...
def local_gates(
    ...
    # 参照量缺席时取空 dict, 下面的 get 就统一落到 False, 无需逐条判 None
    matvec_gates = (
        contract.matvec_reference_gates(matvec) if matvec is not None else {}
    )
    ...
    return {
        ...
        "raw_matvec": referenced_gate(matvec_gates.get("raw_matvec", False)),
        "dirichlet_matvec": referenced_gate(
            matvec_gates.get("dirichlet_matvec", False)
        ),
        "positive_definite": referenced_gate(
            matvec_gates.get("positive_definite", False)
        ),
        "explicit_solution": referenced_gate(
            direct is not None
            and contract.explicit_solution_gate(direct["relative_error"])
        ),
    }
...
def build_payload(
    ...
    return {
        ...
        "parameters": {
            ...
            "material": {
                "hypothesis": material_hypothesis,
