...
使用方法:
    # 默认即充分收敛配置: 2000 组训练样本, 4000 轮训练, 100 组留出样本, 约 17 秒.
    # 这组配置是 experiments/piml_capability 图 3 panel (c) 的证据来源, 不要为了
    # 省时间调小: 300 组 / 400 轮的欠拟合模型柔度误差达 37%, 与图上的 3.64% 差一个
    # 数量级, 据此重跑会误判为记录有误.
...
# 训练样本的密度取值区间与投产几何统一取自 deployment_config.py; 评估密度场必须落在
# DENSITY_RANGE 内, 否则代理工作在外插区间.
...
def main() -> None:
    ...
    parser.add_argument(
        "--train-samples", type=int, default=2000, help="随机密度训练样本数"
    )
