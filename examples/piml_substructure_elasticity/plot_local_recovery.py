...
``outputs/eq17_second_order.json``: 留出集上形函数误差 8.97%, 经式 (17) 后的缩聚
刚度误差 0.435%, 解层全场位移 0.153%。契约与推导见 ``results_analysis.md`` §3。
...
def run_local_recovery_pipeline(save_subfigs: bool = False) -> Dict[str, Any]:
    ...
    print(f"子结构网格     : {n_fine[0]} x {n_fine[1]} Q4 细单元 (共 {n_elements} 单元)")
