    ...
    for ely = 1:nely
        for elx = 1:nelx
            ...
            von_mises(stress_counter, 1) = ...
                sqrt(stress_val(1, 1)^2 + stress_val(2, 1)^2 + 3*stress_val(3, 1)^2 - stress_val(1, 1)*stress_val(2, 1));
            derivative_1 = p * von_mises(stress_counter, 1)^(p-1);
            derivative_2 = [2*stress_val(1, 1) - stress_val(2, 1); 
                            2*stress_val(2, 1) - stress_val(1, 1); 
                            6*stress_val(3, 1)] ./ (2*von_mises(stress_counter, 1));
