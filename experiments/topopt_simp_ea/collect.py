...
def load_case_result(case: TopOptCase) -> dict[str, Any]:
    ...
    if summary.get("method") != "EA-SIMP" or summary.get("assembly_level") != "element":
        raise ValueError(f"运行 {case.run_id} 不是 EA-SIMP matrix-free 口径.")
...
def write(snapshot: dict[str, Any], path: Path | None = None) -> Path:
    target = path or FIGURE_DATA_DIR / "ea_topopt_summary.json"
