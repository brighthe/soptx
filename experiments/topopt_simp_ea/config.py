...
id, 第二层由规范化的 override 标签推导 (见 ``TopOptCase.run_id``), 两侧用相同
override 跑就落在相同路径下, compare 据此配对。
"""
...
FA_EXPERIMENT_DIR = EXPERIMENT_DIR.parent / "topopt_simp_fa"

# 非工况参数: MESH_LAYOUT 是网格生成路径的溯源标签 (真正的生成入口写死在
# pipeline._create_mesh), 后两项是 collect.py 的事后验收阈值; 三者都不进入
# 求解, 因此不进 cases.toml。所有影响求解的参数一律逐工况显式写在注册表里。
...
class TopOptCase:
    ...
    cg_rtol: float = _spec("D", _as_float, positive=True)
    cg_atol: float = _spec("D", _as_float, positive=True)
    cg_maxiter: int = _spec("D", _as_int, positive=True)
    ...
    fa_reference: str = _spec(LEDGER_AXIS, _as_str, missing="")
    # 命令行 override 的原文 (字段名, 文本), 按字段名排序; 空表示注册表基准运行。
    # 只做溯源与 FA 侧重放, 参数本身已经写进对应字段。
...
def parse_override_args(groups: list[list[str]]) -> dict[str, str]:
    ...
    同名键后写覆盖先写。driver.py 与 compare.py 共用, 保证同一串命令行在两处
    解析出同一组 override。
    """
