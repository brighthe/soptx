...
`ua` 对 `pa` 的判据还要更硬。两者的几何量同出 `soptx/fem/levels/_quadrature.py` 的 `quadrature_geometry`，之后走同一串 einsum，浮点运算次序字面上一致，只差在 `pa` 建表时算一次、`ua` 每次作用现算，于是 $L^2$ 误差列应当逐位相同，而不只是吻合到求解容差。哪怕末位有差，都说明两条路径已经岔开。
