...
def _resolve_artifact(case: config.Case) -> Path:
    if case.artifact_path.is_file():
        return case.artifact_path
    
    fallback_map = {
        "a-lagrange-2d": config.REPOSITORY_ROOT / "examples/substructure_elasticity/outputs/lagrange_comparison_2d.json",
        "a-lagrange-3d": config.REPOSITORY_ROOT / "examples/substructure_elasticity/outputs/lagrange_comparison_3d.json",
        "b-shape-route": config.REPOSITORY_ROOT / "examples/piml_substructure_elasticity/outputs/eq17_second_order.json",
        "b-piml-exact": config.REPOSITORY_ROOT / "examples/piml_substructure_elasticity/outputs/piml_exact_comparison.json",
    }
...
def build(cases: tuple[config.Case, ...], figure: dict[str, Any]) -> dict[str, Any]:
    ...
    # --- Panel (a): 纯力学代数等价性 ---
    case_2d = by_id["a-lagrange-2d"]
    case_3d = by_id["a-lagrange-3d"]
    ...
    # --- 数据源读取 ---
    case_shape = by_id["b-shape-route"]
    case_exact = by_id["b-piml-exact"]
    ...
    if not gpu_speedup_path.is_file():
        alt_path = config.PROJECT_ROOT / "examples" / "piml_substructure_elasticity" / "outputs" / "piml_gpu_speedup.json"
        if alt_path.is_file():
    ...
    artifacts_digest = {
        c.id: provenance.file_digest(_resolve_artifact(c))
        for c in cases
    }
