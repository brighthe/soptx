# -*- coding: utf-8 -*-
"""``cases.toml`` 的加载与校验 (各实验目录共用).

本模块只负责把注册表读成结构化的 ``Case`` 对象并做静态校验, 不触发任何计算.
各目录的 ``config.py`` 以自己的 ``PANELS`` / 目录常量调用 ``load_cases``;
运行调度见各目录 ``run.py``, 报表呈现见 ``compare.py``.
"""
...
class Case:
    ...
    属性:
        ...
        panel: 所属图面分格, 取所属目录 ``PANELS`` 之一.
        role: 读取角色, 供后处理脚本按此分派提取逻辑.
        ...
        extra: 其余键值 (如 ``n``, ``method``, ``scheme``), 原样保留供 ``run.py`` / ``compare.py`` 使用.
        repository_root: 仓库根, ``script`` 与 ``fixed`` 产物据此解析.
        output_dir: 所属目录的 ``outputs/``.
    """
    ...
    def script_path(self) -> Path:
        """脚本的绝对路径."""
        return self.repository_root / self.script
    ...
    def artifact_path(self) -> Path:
        ...
        其余模式一律落在所属目录 ``outputs/`` 下.
        """
        if self.output_mode == "fixed":
            return self.repository_root / self.artifact
    ...
    def subprocess_env(self, base: dict[str, str] | None = None) -> dict[str, str] | None:
        """返回子进程环境变量.

        参数:
            base: 目录级固定环境变量 (如线程数限制), 先于 ``env`` 合并.

        返回:
            未声明 ``env`` 且无 ``base`` 时返回 ``None`` 表示继承当前环境; 否则返回合并后的完整环境.
        """
        import os

        if not self.env and not base:
            return None
...
def load_cases(
    ...
    *,
    ...
    repository_root: Path,
    output_dir: Path,
) -> tuple[dict[str, Any], tuple[Case, ...]]:
    """读取并校验 ``cases.toml``.

    参数:
        cases_file: 注册表路径.
        panels: 所属目录合法的 ``panel`` 取值.
        repository_root: 仓库根.
        output_dir: 所属目录的 ``outputs/``.
    ...
    异常:
        ConfigError: 缺少必填键、``id`` 重复、``panel``/``output_mode`` 取值非法,
            或 ``script`` 指向的文件不存在.
    """
    payload = tomllib.loads(cases_file.read_text(encoding="utf-8"))

    figure = payload.get("figure", {})
    raw_cases = payload.get("cases", [])
    if not raw_cases:
        raise ConfigError(f"{cases_file}: 未注册任何 [[cases]]")
    ...
    for raw in raw_cases:
        ...
        if panel not in panels:
            raise ConfigError(f"{case_id}: panel 只能取 {tuple(panels)}, 实得 {panel!r}")
