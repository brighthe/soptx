# -*- coding: utf-8 -*-
"""``cases.toml`` 的加载与校验 (ea_assembly_capability).

本模块只是 ``experiments/_common/casefile.py`` 的目录级薄封装: 提供本目录的路径常量与
...
# EA 数据点的分格: 单刚缓存 / 算子乘 / Jacobi-PCG 求解 / 连续测量, 另加单核硬件基线 (memcpy 带宽 + dgemm 算力).
