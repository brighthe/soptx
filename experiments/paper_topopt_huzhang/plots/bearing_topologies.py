"""不同泊松比下 LFEM 与 HZMFEM 的 k=2 拓扑构型对比 (论文图 5.5).

产物 case ``bearing-topologies``: 2x2 四格, 上排 LFEM 下排 HZMFEM, 左列可压缩基准组
右列近不可压实验组, 四格的运行目录见 REQUIRED_RUNS。

输出: outputs/figures/bearing_topologies.png, 自动同步至 papers/figures/
...
# 相应写全 <case>/<run>, 否则四个同名的 analyzer-*__order-2 分不清属于哪一组。
...
# 面板顺序与 REQUIRED_RUNS 逐项对齐 (先 LFEM 两格再 HZMFEM 两格, 各自左可压缩右近
# 不可压), 下面 zip 起来即成 PANELS: run 路径只在 REQUIRED_RUNS 里写一遍, 不在本文件
# 出现第二处, 免得改一处漏一处。
PANEL_LABELS = [
    ("(a)", "LFEM", 2, r"\nu_0 = 0.30"),
    ("(b)", "LFEM", 2, r"\nu_0 = 0.4999"),
...
def main():
    fig, axes = plt.subplots(2, 2, figsize=(12.0, 5.8), dpi=300)
    axes = axes.flat

    for idx, (tag, method, k, nu_label, case_id, folder_name) in enumerate(PANELS):
        ...
        if vtu_path is None or not vtu_path.is_file():
            ...
            ax.set_title(f"{tag} {method} ($k={k}$, ${nu_label}$)", fontsize=11, fontweight="bold")
            ax.set_xticks([])
            ax.set_yticks([])
            continue
        ...
        ax.set_title(f"{tag} {method} ($k={k}$, ${nu_label}${c_str})", fontsize=11, fontweight="bold", pad=5)

    # 紧凑优雅排版 (完全无 Colorbar 遮挡)
    fig.subplots_adjust(left=0.02, right=0.98, top=0.92, bottom=0.04, wspace=0.08, hspace=0.28)

    save_figure(fig, "bearing_topologies")
    plt.close(fig)
