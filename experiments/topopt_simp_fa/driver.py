# -*- coding: utf-8 -*-
"""FA 变密度拓扑优化的执行驱动: 跑一次运行, 写这一次运行的全部产物.
...
from pipeline import FORMULATION, MESH_LAYOUT, build_pipeline
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help="覆盖线性求解器类型 (mumps, cg, scipy)",
    )
    parser.add_argument(
        "--precond",
        "--cg-precond",
        dest="precond",
        choices=sorted(VALID_PRECONDS),
        default=None,
        help="覆盖 CG 预条件子类型 (none, jacobi)",
    )
    parser.add_argument(
        "--rtol",
        "--cg-rtol",
    ...
    parser.add_argument(
        ...
        dest="grid",
        default=None,
        help="覆盖网格剖分 (如 160,100 或 60,20,4)",
    )
    parser.add_argument(
        "--override",
        action="append",
        ...
        "与基准运行同属一个工况目录; EA 侧用同一组 override 即可配对 compare",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 一级快捷参数自动映射为对应的 overrides 键值
    if arguments.solver is not None:
        overrides["solve_method"] = arguments.solver
    if arguments.precond is not None:
        overrides["cg_precond"] = arguments.precond
    if arguments.rtol is not None:
        overrides["cg_rtol"] = f"{arguments.rtol:g}"
    if arguments.maxiter is not None:
        overrides["cg_maxiter"] = str(arguments.maxiter)
    if arguments.penalty is not None:
        overrides["simp_penalty"] = f"{arguments.penalty:g}"
    if arguments.grid is not None:
        overrides["grid"] = str(arguments.grid)
