...
文件与 ``collect.py``: 本文件挑运行、派发给驱动, collect.py 把已有产物整理成
验收结论:

- 配置          ``config.py`` (路径/TOML 加载/字段声明与校验) ``provenance.py`` (溯源戳记);
- 组装          ``pipeline.py``: soptx 公共组件组装;
- 驱动          ``driver.py``: 跑一次运行、写这一次的全部产物;
- 产出层        ``collect.py``: 结果验收与汇总.
...
``--override`` 在基准上改字段, 产物落在 outputs/<id>/<字段>-<值>[__...]/::
    ...
    python run.py --case <id> --solver cg --precond jacobi --rtol 1e-6
    python run.py --case <id> --override simp_penalty=4.0
    python run.py --case <id> --override solve_method=mumps

作用在已有产物上的验收与汇总归 ``collect.py``::
