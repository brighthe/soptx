...
fealpy 缺少的能力优先在 fealpy_stable 中增强，soptx 调用增强后的接口。通用算法（求解器、算子适配）进 fealpy；soptx 特有的物理模型保留在 soptx。改动同样写入 `docs/known-issues/`。
...
- 每个主题 example 目录统一三文档模板：`README.md`（入口/文件职责）、`math_spec.md`（代码—数学符号映射契约）、`results_analysis.md`（实验证据报告，放目录根，不放 `outputs/`）；`outputs/` 是纯运行产物，由 `.gitignore` 统一忽略
