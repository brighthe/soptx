...
class NHDispNHStressMixedBC(PDEBase):
    """
    二维线弹性 (胡张混合元) —— 非齐次位移边界 + 非齐次应力边界条件
...
class HDispNHStressMixedBdcc(PDEBase):
    """
    二维线弹性 (胡张混合元) —— 齐次位移边界 + 非齐次应力边界条件
    ...
    解析应力 (λ=1, μ=0.5):
        σ_xx = 2π cos(πx) sin(πy) + π sin(πx) cos(πy)
        σ_xy =  (π/2)[ sin(πx) cos(πy) + cos(πx) sin(πy) ]
        σ_yy =  π cos(πx) sin(πy) + 2π sin(πx) cos(πy)

    体力密度:
        b_x = b_y = (5π²/2) sin(πx) sin(πy) - (3π²/2) cos(πx) cos(πy)

    边界条件:
        位移边界 Γ_D = {x=0} ∪ {y=0}: u_D = 0  (自然边界条件, 弱施加)
        牵引边界 Γ_N = {x=1} ∪ {y=1}: σ·n = g_N (本质边界条件, 强施加)
            右边界 x=1, n=(1,0): g_N = [ -2π sin(πy),   -(π/2) sin(πy) ]^T
            上边界 y=1, n=(0,1): g_N = [ -(π/2) sin(πx), -2π sin(πx)   ]^T
    """
    def __init__(self, 
            domain: List[float] = [0, 1, 0, 1],
            mesh_type: str = 'uniform_crisscross_tri', 
    ...
    @cartesian
    def displacement_solution(self, points: TensorLike) -> TensorLike:
        """位移解析解 u(x,y) = [sin(πx)sin(πy), sin(πx)sin(πy)]^T"""
        x, y = points[..., 0], points[..., 1]
        pi = bm.pi
        val = bm.sin(pi * x) * bm.sin(pi * y)
        return bm.stack([val, val], axis=-1)
    ...
    def grad_displacement_solution(self, points: TensorLike) -> TensorLike:
        ...
        grad = bm.stack([du_dx, du_dy], axis=-1)
        return bm.stack([grad, grad], axis=-2)
    ...
    def stress_solution(self, points: TensorLike) -> TensorLike:
        ...
        eps_xx = pi * bm.cos(pi * x) * bm.sin(pi * y)
        eps_yy = pi * bm.sin(pi * x) * bm.cos(pi * y)
        eps_xy = 0.5 * pi * (bm.sin(pi * x) * bm.cos(pi * y)
    ...
    @cartesian
    def body_force(self, points: TensorLike) -> TensorLike:
        """体力密度 b(x,y) = (5π²/2)sin(πx)sin(πy) - (3π²/2)cos(πx)cos(πy)"""
        x, y = points[..., 0], points[..., 1]
        pi = bm.pi
        val = (5.0 / 2.0) * pi**2 * bm.sin(pi * x) * bm.sin(pi * y) \
            - (3.0 / 2.0) * pi**2 * bm.cos(pi * x) * bm.cos(pi * y)
        return bm.stack([val, val], axis=-1)
    ...
    def is_displacement_boundary(self, points: TensorLike) -> TensorLike:
        """位移边界 Γ_D = {x=0} ∪ {y=0}，u_D=0 (自然边界条件，弱施加)"""
        x, y = points[..., 0], points[..., 1]
        return (bm.abs(x - self._domain[0]) < self._eps) | \
               (bm.abs(y - self._domain[2]) < self._eps)

    def is_traction_boundary(self, points: TensorLike) -> TensorLike:
        """牵引边界 Γ_N = {x=1} ∪ {y=1} (本质边界条件，强施加)"""
        x, y = points[..., 0], points[..., 1]
        return (bm.abs(x - self._domain[1]) < self._eps) | \
               (bm.abs(y - self._domain[3]) < self._eps)

    @cartesian
    def displacement_bc(self, points: TensorLike) -> TensorLike:
        """位移边界条件: 齐次, u_D = 0
        
        NOTE: 当前 assemble_displacement_bc_vector 直接返回零向量，
              与此处 u_D=0 一致，无需修改求解器代码
        """
        return bm.zeros(points.shape, dtype=bm.float64)

    # @cartesian
    # def traction_bc(self, points: TensorLike) -> TensorLike:
    #     """牵引边界条件: 返回精确应力张量 σ 在边界上的值
        
    #     右边界 x=1, n=(1,0): g_N = [ π sin(πy),   (π/2) sin(πy) ]^T
    #     上边界 y=1, n=(0,1): g_N = [ (π/2) sin(πx), π sin(πx)   ]^T
    #     """
    #     return self.stress_solution(points)
    
    @cartesian
    def traction_bc(self, points: TensorLike) -> TensorLike:
        """牵引力向量 [t_x, t_y] = σ·n 在全局坐标系下的分量"""
        x, y = points[..., 0], points[..., 1]
        pi = bm.pi

        kwargs = bm.context(points)
        val = bm.zeros((*points.shape[:-1], 2), **kwargs)

        flag_right = bm.abs(x - self._domain[1]) < self._eps
        val = bm.set_at(val, (flag_right, 0), -2.0 * pi * bm.sin(pi * y)[flag_right])
        val = bm.set_at(val, (flag_right, 1), -(pi / 2.0) * bm.sin(pi * y)[flag_right])

        flag_top = bm.abs(y - self._domain[3]) < self._eps
        val = bm.set_at(val, (flag_top, 0), -(pi / 2.0) * bm.sin(pi * x)[flag_top])
        val = bm.set_at(val, (flag_top, 1), -2.0 * pi * bm.sin(pi * x)[flag_top])
